...
mod tests {
    ...
    fn test_fresh_patcher_match() {
        ...
        // from debug-run (generated by the SAME ClientPatcher.exe binary).
        // Uses texture 5792765 (256x256 DXT1) which is available in debug-run.
        use crate::rdb::parse_le_index;
        use std::io::{Read, Seek, SeekFrom};
        use std::path::PathBuf;
        ...
        // Determine dimensions from FCTX header
        // Byte 16-17 = mip count (per dimension)
        ...
        let block_size = 8;
        let fctx_hdr_size = 24;
        let mip0_size = (width/4)*(height/4)*block_size; // 32768
        let mip1_size = (width/8)*(height/8)*block_size; // 8192

        // Extract mip0 and mip1 from FCTX (smallest-first layout)
        ...
        for by in 0..(nh/4) { for bx in 0..nbpw {
            let mut bp = [[0u8; 4]; 16];
            for py in 0..4 { for px in 0..4 {
        ...
        let matching = (0..total).filter(|&b| our_mip1[b*8..(b+1)*8] == ref_mip1[b*8..(b+1)*8]).count();
        let solid = (0..total).filter(|&b| {
            u16::from_le_bytes([ref_mip1[b*8], ref_mip1[b*8+1]]) ==
        ...
        for b in 0..total.min(100) {
            let ref_block = &ref_mip1[b*8..(b+1)*8];
            let our_block = &our_mip1[b*8..(b+1)*8];
            let ref_px = decode_dxt1_block(ref_block);
            let our_px = decode_dxt1_block(our_block);
            for i in 0..16 {
                for c in 0..3 {
                    let d = (ref_px[i][c] as i32 - our_px[i][c] as i32).unsigned_abs();
                    total_diff += d as u64;
        ...
        let avg = total_diff as f64 / pixel_count as f64;
        eprintln!("    Pixel analysis (first 100 blocks): avg_diff={:.2} max_diff={}", avg, max_diff);
        // Show first 3 non-matching blocks
        for b in 0..total.min(3) {
            let rc0 = u16::from_le_bytes([ref_mip1[b*8], ref_mip1[b*8+1]]);
            let rc1 = u16::from_le_bytes([ref_mip1[b*8+2], ref_mip1[b*8+3]]);
            let oc0 = u16::from_le_bytes([our_mip1[b*8], our_mip1[b*8+1]]);
            let oc1 = u16::from_le_bytes([our_mip1[b*8+2], our_mip1[b*8+3]]);
            let rix = u32::from_le_bytes([ref_mip1[b*8+4], ref_mip1[b*8+5], ref_mip1[b*8+6], ref_mip1[b*8+7]]);
            let oix = u32::from_le_bytes([our_mip1[b*8+4], our_mip1[b*8+5], our_mip1[b*8+6], our_mip1[b*8+7]]);
